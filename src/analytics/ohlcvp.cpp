...
public:
    ...
    [[nodiscard]] bool WriteOhlcvp(const std::pair<uint64_t,std::vector<double>>& vOhlcvp);
};

Ohlcvp::DB::DB(const fs::path& path, std::string column_name) :
    BaseAnalytic::DB(path,column_name)
{}
//gArgs.GetDataDirNet() "/analytics"


bool Ohlcvp::DB::WriteOhlcvp(const std::pair<uint64_t,std::vector<double>>& vOhlcvp)
{
...
bool Ohlcvp::CustomAppend(const interfaces::BlockInfo& block)
{
    ...
    auto ohlcvp = CalculateOHLCVP(*block.data,block_undo, btc_price_map);
    /* point2 = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> duration = point2 - point1;
    if (perf_stream.is_open()) {
        perf_stream << "Timing of CalculateOHLCVP: " << duration.count() <<"\n";
    } else {
        std::cerr << "Error: log file is not open." << std::endl;
    }*/
    

    if (!ohlcvp.has_value()) { return true;} //price missing, skip to next

    std::pair<uint64_t,std::vector<double>> vOhlcvp = std::make_pair(blockTime,std::vector<double>{ohlcvp.value()}); 

    return m_db->WriteOhlcvp(vOhlcvp);
}

BaseAnalytic::DB& Ohlcvp::GetDB() const { return *m_db; }

std::optional<double> Ohlcvp::CalculateOHLCVP(const CBlock& block, const CBlockUndo& blockUndo, const std::unordered_map<int64_t, double>& btc_price_map) {
...
std::unordered_map<int64_t, double> Ohlcvp::LoadBTCPrices(const std::string& file_path){
...
std::optional<double> Ohlcvp::GetBTCPrice(const std::unordered_map<int64_t, double>& btc_price_map, 
                                  int64_t timestamp, 
...
std::unordered_map<int64_t, double> Ohlcvp::ConvertTableToMap(const std::shared_ptr<arrow::Table>& table) {
