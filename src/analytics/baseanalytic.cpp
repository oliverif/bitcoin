...
BaseAnalytic::DB::DB(const fs::path& path) : m_db_path(path)
{
    ...
    // Get the column name from GetName()
    std::string column_name = GetName();

    // Check if the analytics table exists
    const char* check_table_sql = "SELECT name FROM sqlite_master WHERE type='table' AND name='analytics';";
    sqlite3_stmt* stmt;
    if (sqlite3_prepare_v2(m_db, check_table_sql, -1, &stmt, nullptr) == SQLITE_OK) {
        if (sqlite3_step(stmt) == SQLITE_ROW) {
            // The table exists, check if the column exists
            ...
            CreateAnalyticsTable(column_name);
        }
    }
...
bool BaseAnalytic::DB::ColumnExists(const std::string& column_name) {
    std::string query = "PRAGMA table_info(analytics);";
    sqlite3_stmt* stmt;
...
bool BaseAnalytic::DB::ReadBestBlock(CBlockLocator& locator) const
{
    ...
    sqlite3_bind_text(stmt, 1, GetName().c_str(), -1, SQLITE_TRANSIENT); // Use GetAnalyticId() directly
    if (sqlite3_step(stmt) == SQLITE_ROW) {
        const void* blob = sqlite3_column_blob(stmt, 0);
        int blob_size = sqlite3_column_bytes(stmt, 0);
        std::istringstream iss(std::string(static_cast<const char*>(blob), blob_size));
        locator.Unserialize(iss);
        sqlite3_finalize(stmt);
        return true;
    }
    sqlite3_finalize(stmt);
...
void BaseAnalytic::DB::WriteBestBlock(const CBlockLocator& locator)
{
    ...
    locator.Serialize(oss);
    std::string locator_blob = oss.str();
...
void BaseAnalytic::DB::WriteAnalytics(const std::pair<uint64_t, std::vector<double>>& analytics) {
    ...
    std::string sql = "INSERT INTO analytics (timestamp, " + column_name + ") VALUES (?, ?);";
    ...
    if (!values.empty()) {
        sqlite3_bind_double(stmt, 2, values[0]); // Bind the first value
    } else {
